...
for (var npc in dnd.npcs) {
    dnd.npcs[npc].name = npc;
    if (dnd.npcs[npc].content) {
        for (var i = 0; i < dnd.npcs[npc].content.length; i++) {
            var content = dnd.npcs[npc].content[i];
            if (typeof(content) == 'string' && content.match(/\*challenge\*/i)) {
                var values = content.match(/\*\*\s+([\d\\\/]+)\s+\(([\d\,]+)/);
                if (values) {
                    dnd.npcs[npc].cr = values[1];
                    dnd.npcs[npc].xp = values[2];
                }
            }
        }
    }
